      ...
      <main>
        ...
        <section className="border-b border-[var(--edge)]">
          <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            <ScrollReveal>
              <Link href="/people" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--forge-amber)] hover:text-[var(--arc-blue)] transition-colors">
              ...
              <div className="grid gap-6 md:grid-cols-[180px_minmax(0,1fr)] md:gap-8">
                ...
                <ScrollReveal delay={100}>
                  <div className="min-w-0">
                    ...
                    <h1 className="mt-3 overflow-wrap-anywhere font-display text-4xl font-bold leading-tight sm:text-5xl text-[var(--ceramic)]">{person.name}</h1>
                    <p className="mt-2 text-base font-semibold text-[var(--arc-blue)] sm:text-lg">{person.role}</p>
                    <p className="mt-4 max-w-3xl text-base leading-7 text-[var(--ceramic-muted)] sm:text-lg sm:leading-8">{person.synopsis}</p>
                    <div className="mt-6">
                  ...
                  {person.projects.length ? (
                    person.projects.map((project) => (
                      <GlassCard key={project.title} variant="featured" accent="blue">
                        ...
                        <span className="mt-2 inline-block font-data text-[10px] uppercase tracking-[0.14em] text-[var(--forge-amber)]">{project.status}</span>
                      </GlassCard>
                    ))
                  ) : (
